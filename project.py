...
class Obs:
    ...
    def plot_tauint(self, save=None):
        ...
        for e, e_name in enumerate(self.mc_names):
            ...
            # adding a horizontal line at tau_int at the window size
            plt.axhline(y=self.e_n_tauint[e_name][self.e_windowsize[e_name]], color='C' + str(e), alpha=0.5, marker=',', ls='--')
            # printing the window size on the plot on the left side
            plt.text(self.e_windowsize[e_name]+1, 0.5, str(self.e_windowsize[e_name]), color='C' + str(e))
            # printing the corresponding tau_int on the plot on the left side
            plt.text(1, self.e_n_tauint[e_name][self.e_windowsize[e_name]]+0.1, str(round(self.e_n_tauint[e_name][self.e_windowsize[e_name]],2)), color='C' + str(e))
            plt.legend()
            plt.xlim(-0.5, xmax)
            ylim = plt.ylim()
            plt.ylim(bottom=0.0, top=max(1.0, ylim[1]))
            plt.draw()
            print('Window size for beta = ', e_name, ':', self.e_windowsize[e_name], 'Tau_int = ', round(self.e_n_tauint[e_name][self.e_windowsize[e_name]],2))
            if save:
